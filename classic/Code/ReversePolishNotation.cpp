...
private:
    bool isop(string str){
        if (str.compare("*")==0||str.compare("/")==0||str.compare("+")==0||str.compare("-")==0) return true;
        return false;
    }
    int op(string a , string op, string b){
        if(op.compare("*")==0)return atoi(a.c_str())*atoi(b.c_str());
        if(op.compare("/")==0)return atoi(a.c_str())/atoi(b.c_str());
        if(op.compare("+")==0)return atoi(a.c_str())+atoi(b.c_str());
        if(op.compare("-")==0)return atoi(a.c_str())-atoi(b.c_str());
    }
    ...
    int evalRPN(vector<string> &tokens) {
		...
		cout<<tokens[i]<<endl;
                ...
                if(cache.size()>=2){
                    ...
                    cache.pop();
                    ostringstream ss;
                    ss << op(s1,tokens[i],s2);
                    cache.push(ss.str());
                }
            }
        }
        return atoi(cache.top().c_str());
    }
};
